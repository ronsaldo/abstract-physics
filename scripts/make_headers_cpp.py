...
class MakeHeaderVisitor:
    ...
    def emitMethodWrapper(self, function):
        ...
        if function.returnType == 'error':
            self.printLine('\tinline void $FunctionName ( $Arguments )',
                ...
                Arguments = arguments)
            self.printLine('\t{')
            self.printLine('\t\tAPhyThrowIfFailed($FunctionPrefix$FunctionName( $Arguments ));', FunctionName = function.cname, Arguments = paramNames)
            self.printLine('\t}')
            self.newline()
        else:
    ...
    def emitInterface(self, interface):
        ...
        self.printLine('};')
        self.newline()
        self.printLine('typedef aphy_ref<$TypePrefix$Name> $TypePrefix${Name}_ref;', Name = interface.name)
        self.newline()
